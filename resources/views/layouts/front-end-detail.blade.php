...
<div class="site-wrap">
    ...
    <div class="intro-section" id="home-section">
        ...
        <div class="slide-1" style="background-image: url({{url("front-end/images/hero_1.jpg")}});"
                    ...
                    <div class="col-12">
                        <div class="row align-items-center">
                            <div class="col-lg-6 mb-4">
                                ...
                                <p class="mb-4" data-aos="fade-up" data-aos-delay="200">Wisata merupakan tempat orang
                                    berlibur, dimana tempat tersebut dapat menambah wawasan dan dapat kebahagian
                                    tersendiri.</p>
                                <p data-aos="fade-up" data-aos-delay="300"><a href="#courses-section"
                                                                              class="btn btn-primary py-3 px-5 btn-pill">Jelajahi
                                        Wisata</a></p>
    ...
    <div class="site-section" id="programs-section">
        <div class="container">
            <div class="row mb-3 justify-content-center">
                <div class="col-lg-7 text-center" data-aos="fade-up" data-aos-delay="">
                    <h3 class="section-title text-muted">Detail</h3>
            ...
            <div class="row mb-3 align-items-center">
                <div class="col-lg-7 mb-5" data-aos="fade-up" data-aos-delay="100">
                    <img src="{{url('gambar')}}/{{$detail->foto_wisata}}" alt="Image" class="img-fluid">
                    <p class="text-info text-center"><b>Deskirpsi : </b> <br><i>"{!! $detail->deskripsi_wisata !!}"</i></p>
                </div>
                <div class="col-lg-4 ml-auto" data-aos="fade-up" data-aos-delay="200">
                    <h2 class="text-black mb-4">{{$detail->nama_wisata}}</h2>
                    <h5 class="text-info">Detail Wisata</h5>
                    <p class="mb-4">Daerah : {{$detail->daerah_wisata}}<br>
                        Kategori : {{$detail->kategori_wisata}}<br>
                        Harga : {{number_format($detail->harga_tiket_wisata)}}<br>
                        Pengunjung : {{number_format($detail->pengunjung_wisata)}}<br>
                        Fasilitas : {{number_format($detail->fasilitas_wisata)}}%<br>
                        Pelayanan : {{number_format($detail->pelayanan_wisata)}}%<br>
                        Jarak dari kota : {{number_format($detail->jarak_wisata)}} Meter<br>
                    </p>

                    <div class="d-flex align-items-center custom-icon-wrap mb-3">
                        <span class="custom-icon-inner mr-3"><span class="icon icon-envelope"></span></span>
                        <div>
                            <h3 class="m-0">{{number_format(\App\TopsisModel::CountKomen(array("wisata_id" =>$detail->wisata_id)))}}
                                Komentar</h3></div>
                    </div>
                ...
                <div class="container">
                    ...
                    <div class="row justify-content-center">
                        <div class="col-md-7">
                            ...
                            <h4 class="text-center mb-3 text-muted">Beri Komentar Terbaik</h4>
                                ...
                                <div class="form-group row">
                                    <div class="col-md-12">
                                        <input type="text" name="nama" class="form-control"
                                               placeholder="Nama Lengkap (tidak wajib)">
                                        <input type="hidden" name="wisata_id" value="{{$detail->wisata_id}}" required>
                                    </div>
                                </div>
                                ...
                                <div class="form-group row">
                                    <div class="col-md-12">
                                                <textarea name="komentar" class="form-control" id="" cols="30" rows="5"
                                                          placeholder="ketik komentar terbaik Anda disini."
                                                          required></textarea>
                                    </div>
                                </div>

                                <div class="form-group row">
                                    <div class="col-md-6 mx-auto notif_komentar">
                                        <a class="btn btn-primary py-3 px-5 btn-block btn-pill text-white"
                                           onclick="kirimKomentar()">Kirim Komentar</a>
                                    </div>
                                </div>

                            </form>
                            <div class="form-group tampil_komentar">
                                @foreach(\App\TopsisModel::getKomen(array("wisata_id" => $detail->wisata_id)) as $value)
                                    <p class="text-info"><i class="icon-user-circle"></i> - {{$value->nama}}</p>
                                    <p class="text-info"><i class="icon-envelope"></i><i> "{!! $value->komentar !!}"</i></p>
                                    <small class="text-muted"><i class="icon-clock-o"> {{$value->created_at}}</i></small>
                                    <hr>
                                @endforeach
                            </div>
                        </div>
...
<script>
    function kirimKomentar() {
        var wisata_id = $('[name="wisata_id"]').val();
        var nama = $('[name="nama"]').val();
        var komentar = $('[name="komentar"]').val();
        // alert(wisata_id+komentar+nama);

        $.ajax({
            url: '{{route("save_komen")}}',
            method: 'POST',
            dataType: 'JSON',
            data: {_token: '{{csrf_token()}}', wisata_id: wisata_id, nama: nama, komentar: komentar},
            success: function (result) {
                if (result.status === true) {
                    // $(".notif_komentar").append("<p class='text-success'>Terimkasih, komentar Anda  berhasil di kirim</p>");
                    // $(".tampil_komentar").load();
                    alert("Terimkasih, komentar Anda  berhasil di kirim");
                    window.location.reload();
                } else {
                    $(".notif_komentar").append("<p class='text-danger'>Ops.., komentar Anda  gagal di kirim, silahkan coba lagi nanti.</p>");
                }
            }
        })
    }
